...
def mods_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sync_mods, "MODS_DIR", manifest_dir)
    monkeypatch.setattr(sync_mods, "MANIFEST_PATH", manifest_dir / "mods-manifest.json")
    return manifest_dir
...
def test_resolve_modrinth_version_not_found(monkeypatch):
    ...
    monkeypatch.setattr(sync_mods, "api_get", lambda _path: [{"version_number": "1.0.0"}])
...
def test_resolve_download_routes(monkeypatch):
    monkeypatch.setattr(sync_mods, "resolve_modrinth", lambda *_args: ("u", "h"))
    monkeypatch.setattr(sync_mods, "resolve_curseforge", lambda *_args: ("c", "d"))
...
def test_api_get(monkeypatch):
    ...
    monkeypatch.setattr(sync_mods.requests, "get", lambda *_args, **_kwargs: FakeResponse())
...
def test_main_manifest_missing(monkeypatch, tmp_path):
    monkeypatch.setattr(sync_mods, "MANIFEST_PATH", tmp_path / "missing.json")
...
def test_main_empty_mods(mods_dir):
    sync_mods.MANIFEST_PATH.write_text(json.dumps({"mods": []}), encoding="utf-8")
    sync_mods.main()


def test_main_with_failure(mods_dir, monkeypatch):
    sync_mods.MANIFEST_PATH.write_text(
...
def test_load_manifest(mods_dir):
    payload = {"mods": [{"id": "a"}]}
    sync_mods.MANIFEST_PATH.write_text(json.dumps(payload), encoding="utf-8")
