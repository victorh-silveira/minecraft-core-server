...
def test_sha256_digest_normalizes_and_presence() -> None:
    digest = Sha256Digest("  ABC ")
    assert digest.value == "abc"
    assert digest.is_present()
    assert not Sha256Digest("").is_present()
    assert digest == Sha256Digest("abc")
    assert digest != Sha256Digest("def")
...
def test_download_artifact_requires_url() -> None:
    ...
    artifact = DownloadArtifact(" https://cdn/mod.jar ", Sha256Digest("ab"))
...
def test_jar_filename_and_checksum() -> None:
    ...
    expected = Sha256Digest("aa")
    assert matches_expected(Sha256Digest("aa"), expected)
    assert not matches_expected(Sha256Digest("bb"), expected)
    assert matches_expected(Sha256Digest("bb"), Sha256Digest(""))
