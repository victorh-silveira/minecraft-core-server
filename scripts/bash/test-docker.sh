...
if [[ -f .env ]]; then
  ...
  fail ".env ausente (copie .env.example para .env)"
fi
...
if [[ -f .env ]] && docker compose --env-file .env config --quiet >/dev/null 2>&1; then
...
if command -v bash >/dev/null 2>&1; then
  if timeout 3 bash -c "echo > /dev/tcp/127.0.0.1/${GAME_PORT}" 2>/dev/null; then
    ...
    warn "porta ${GAME_PORT} nao respondeu em 127.0.0.1 (pode ser normal no Docker Desktop/WSL)"
  fi
...
if [[ -f src/interface/mods/mods-manifest.json ]]; then
  pass "mods-manifest.json presente"
  JAR_COUNT="$(find src/interface/mods -maxdepth 1 -name '*.jar' 2>/dev/null | wc -l | tr -d ' ')"
  if [[ "${JAR_COUNT:-0}" -gt 0 ]]; then
    pass "${JAR_COUNT} jar(s) em src/interface/mods"
  else
    warn "nenhum jar em src/interface/mods (rode: make docker-sync-mods)"
  fi
else
...
if docker ps --format '{{.Names}}' | grep -qx "$CONTAINER"; then
  docker compose --env-file .env logs --tail=15 "$SERVICE" 2>/dev/null | sed 's/^/     /'
  if docker compose --env-file .env logs --tail=80 "$SERVICE" 2>/dev/null | grep -q "Done (.*)! For help, type \"help\""; then
